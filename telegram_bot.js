...
async function handleMessage(msg) {
  ...
  if (text === MENU.guide) {
    ...
    await sendMessage(
      chatId,
      '📘 *Инструкция*\n\n' +
        '1️⃣ *Первичная настройка автопарсинга (рекомендуется сделать один раз):*\n' +
        '   • _⚙️ Настройки автопарсинга_ → _🔧 Настроить фильтры_\n' +
        '   • заполните фильтры (Авито и/или ВБ) и сохраните\n' +
        '   • затем _⏱ Настроить периодичность_ (интервал автообхода ВБ)\n' +
        '   • включите _✅ Автопарсинг ВКЛ_\n' +
        '   • при необходимости включите _📊 Сохранять Excel: ВКЛ_\n\n' +
        '2️⃣ *Как работает автопарсинг ВБ:*\n' +
        '   • первый автообход формирует базу объявлений (без уведомлений)\n' +
        '   • со второго обхода приходят только *новые* объявления\n\n' +
        '3️⃣ *Проверка состояния:*\n' +
        '   • _🔎 Проверить автопарсинг_ показывает: ВКЛ/ВЫКЛ, интервал, время следующего запуска\n\n' +
        '4️⃣ *Ручной запуск:*\n' +
        '   • _🚀 Ручной запуск_ → площадка → режим\n' +
        '   • _▶️ Продолжить с настройками автопарсинга_ — взять сохранённые фильтры\n' +
        '   • _✍️ Задать вручную_ — разовый запуск без изменения сохранённых настроек\n\n' +
        '5️⃣ *Управление во время запуска:*\n' +
        '   • _❌ Отмена_ — выйти из мастера настроек\n' +
        '   • _⛔ Остановить парсинг_ — остановить текущий ручной запуск\n\n' +
        '6️⃣ *Excel-файлы:*\n' +
        '   • _📁 Эксель файлы_ → _👀 Посмотреть_ — отправка файлов в чат\n' +
        '   • _📁 Эксель файлы_ → _🗑 Удалить_ — удаление с сервера и из БД\n\n' +
        '💡 Если автообход не стартует: проверьте _✅ Автопарсинг ВКЛ_, интервал и статус через _🔎 Проверить автопарсинг_.',
      mainKeyboard,
      'Markdown'
    );
    return;
